...
//        http.authorizeHttpRequests(authorize -> authorize.requestMatchers("/actuator/**", "/users/**").permitAll().anyRequest().permitAll())
...
//                mappedAuthorities.addAll(roles);
//            }
//            List<String> scopes = Arrays.stream(jwt.getClaimAsString("scopes").split("\\s+")).map(scope -> "SCOPE_" + scope).collect(Collectors.toList());
